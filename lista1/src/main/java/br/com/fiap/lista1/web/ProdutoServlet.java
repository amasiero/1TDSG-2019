...
public class ProdutoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (usuario.equals("<deslogado>")) {
			out.println("<p> É necessário estar logado para essa funcionalidade.</p>");
		} else {
			Produto p = new Produto(nome, quantidade);
			if (new ProdutoBO().validarProduto(p)) {
				new ProdutoDAO().adicionarProduto(p);
				out.println("<p> O produto " + p.getNome() + " foi cadastrado com sucesso!</p>");
				out.println("<p> Está com " + p.getQuantidadeEmEstoque() + " unidades em estoque.</p>");
			} else {
				out.println("<p>É necessário informar o nome e " + "a quantidade em estoque.</p>");
			}
		}
